...
#defines about model
GAMES = 100 #number of games in unity
HIDDEN_LAYERS = 800
...
class QAgent:
    def __init__(self,  actions_size, batch_size,input_dims,mem_size=1000000,
                 model_file='model_experience_buffer.h5', history_file='history_experience_buffer'):
        ...
        self.model = build_dqn(input_dims, actions_size)
        self.history = []
    ...
    def train(self):
        ...
        his = self.model.fit(states, q_target, verbose=0)

        self.history.append(his.history)

        if self.epsilon > EPS_MIN:
            self.epsilon = EPS_REDUCE*self.epsilon
        else:
